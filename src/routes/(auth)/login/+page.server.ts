...
export const actions = {
  default: async ({ request, cookies }) => {
    ...
    if (!user) return message(form, { status: "error", text: "Incorrect username or password." }, { status: 401 });

    const validPassword = await new Argon2id().verify(user.password, form.data.password);

    if (!validPassword) return message(form, { status: "error", text: "Incorrect username or password." }, { status: 401 });
